...
export const th: Dict = {
  ...
  'time.viewer': 'เวลาของฉัน',
  'time.pickZone': 'เลือกโซนเวลา',
  'time.siteLocalNote': 'เวลาที่แสดงเป็นเวลาท้องถิ่นของแต่ละไซต์ · การคำนวณยังใช้กะของแต่ละไซต์',
  'time.viewerNote': 'เวลาที่แสดงเป็นเวลาเครื่องของคุณ {tz} · การคำนวณยังใช้กะของแต่ละไซต์',
  'time.fixedNote': 'เวลาที่แสดงเป็น {tz} · การคำนวณยังใช้กะของแต่ละไซต์',
  ...
  /* พิลล์ข้างตัวเลขกว้างราว 45px เท่านั้น คำว่า "เป้าหมาย" ถูกตัดเป็น เป้า | หมาย
     จึงขึ้นบรรทัดที่สามและดันการ์ดทั้งแถวให้สูงขึ้น "เป้า" เป็นคำที่โรงงานใช้กันอยู่แล้ว
     และอยู่ในสองบรรทัดเท่ากับภาษาอังกฤษ ส่วนคำเต็มยังใช้ใน drawer ที่มีที่ว่างพอ */
  'kpi.targetShort': 'เป้า {target}%',
  /* ชื่อสถานะมาจากระบบต้นทาง ไม่แปล */
  'kpi.running.definition': 'Mass Pro/Dandori',
  /*
   * แจกแจงบนหน้าการ์ดเลย ไม่ใช่ซ่อนไว้ในแผงที่มา - บอร์ดหน้างาน v4 แยก Dandori
   * ออกจาก RUNNING เป็น tile ของตัวเองแล้ว (F-20) ของเรายังรวมไว้ตามนิยามเดิม
   * คนที่เอาสองจอมาวางข้างกันจึงต้องอ่านออกจากการ์ดได้ทันทีว่า 37 ของเราคือ 34
   * กับ 3 ของบอร์ด ไม่ใช่ตัวเลขคนละเรื่องกัน
   */
  'kpi.running.definition.split': 'Mass Pro {massPro} · Dandori {dandori}',
  /* แก้ 2026-09-10 - ดู en.ts · ประโยคเดิม "ไม่ถูกนับที่ไหนเลย" จริงเฉพาะตอนที่ STOP
     ยังนับแค่สถานะ Stop เท่านั้น */
  'kpi.running.tooltip':
    'เครื่องที่อยู่ในสถานะ Mass Pro หรือ Dandori · 4M Change ยังไม่นับเป็นเดินเครื่อง เพราะยังไม่ได้ตัดสินว่าควรนับหรือไม่ จึงไปรวมอยู่ในการ์ด Stop พร้อมกับสถานะอื่นที่ไม่ได้ผลิต',
  'kpi.stopped.definition': 'หยุดนอกแผน',
  /* ตัวเลขนี้เลิกเป็น "เฉพาะสถานะ Stop" ตั้งแต่ 2026-08-27 - ดู en.ts */
  'kpi.stopped.tooltip':
    'ทุกเครื่องที่นับแล้วแต่ไม่ได้เดิน คือสถานะ Stop รวมกับสถานะอื่นที่ไม่ใช่ Mass Pro หรือ Dandori เช่น No Plan หรือ 4M Change · บรรทัดใต้ตัวเลขแจกแจงให้ว่าเป็นสถานะไหนบ้าง · มีแต่ Order End ที่อยู่นอกทั้งตัวเลขนี้และเครื่องจักรทั้งหมด เพราะบอร์ดหน้างานนับใบสั่งที่ผลิตจบเป็นการ์ดของมันเอง',
  ...
  /* ชื่อการ์ดไม่แปล - เป็นชื่อที่บอร์ดหน้างานใช้อยู่แล้ว (%AR (PROGRESS))
     คนที่อ่านหน้าโรงงานคู่กับบอร์ดจึงเทียบตัวเลขได้ตรง ๆ ส่วนหน้าภาพรวมยังใช้
     '%Achievement' ตามเดิม เพราะผู้บริหารไม่ได้อ่านคู่กับบอร์ด */
  'kpi.ar': '%AR',
  'kpi.ar.tooltip':
    '%AR (Achievement Rate) คือจำนวนที่ผลิตได้เทียบกับจำนวนของใบสั่งที่เครื่องโหลดอยู่ ไม่ใช่เป้าต่อกะหรือต่อวัน เครื่องที่เพิ่งเริ่มใบสั่ง 800 ชิ้นจึงอ่านได้ต่ำทั้งที่ยังทันแผน · หน้าภาพรวมเรียกตัวเลขเดียวกันนี้ว่า %Achievement',
  /* ชื่อสถานะมาจากระบบต้นทาง ไม่แปล */
  'kpi.orderEnd': 'Order End',
  /* คำบรรยายต้องบอกหน่วย เพราะป้ายการ์ดไม่ได้บอก และตัวเลขนี้อยู่ข้างการ์ดที่นับ
     "เครื่อง" อีกสามใบ · "วันนี้" คือวันตามปฏิทินของไซต์ เที่ยงคืนถึงเที่ยงคืน
     ซึ่งเป็นช่วงเดียวกับที่บอร์ดหน้างานนับ */
  'kpi.orderEnd.definition': 'ใบสั่งที่ผลิตจบวันนี้',
  'kpi.orderEnd.tooltip':
    'จำนวนใบสั่งผลิตที่จบไปแล้วตั้งแต่เที่ยงคืนตามเวลาโรงงาน เท่ากับจำนวนการ์ดบนบอร์ดหน้างาน · นับเป็น "ใบสั่ง" ไม่ใช่ "เครื่อง" เครื่องที่ผลิตจบ 3 ใบในวันนี้จะนับเป็น 3 ตรงนี้ แต่เป็น 1 ในการ์ดเครื่องจักรทั้งหมด · ใบสั่งที่จบไม่ใช่ความผิดปกติ เครื่องแค่ขึ้นใบถัดไปหรือกำลังรอใบใหม่',
  'kpi.orderEnd.source':
    'อ่านจากใบสั่งผลิตที่แต่ละเครื่องเดินในวันนี้ ไม่ได้อ่านจากสถานะเครื่อง · ใบสั่งจะถือว่าจบเมื่อเครื่องขึ้นใบใหม่แล้ว ซึ่งเป็นกติกาเดียวกับที่บอร์ดหน้างานใช้ตอนสร้างการ์ดใบที่สองคู่กับใบที่กำลังเดินอยู่ · ส่วนเครื่องที่ส่งสถานะ “Order End” มาเองเพราะใบสั่งหมดแล้วและยังไม่มีใบใหม่ขึ้น ถือเป็นเครื่องที่กำลังรอ ไม่ใช่ใบสั่งที่จบ จึงไม่ถูกนับตรงนี้ และถูกตัดออกจาก %OA เหมือนที่บอร์ดหน้างานทำ',
  'kpi.orderEnd.source.formula': 'Order End = ใบสั่งที่จบตั้งแต่ 00:00 ตามเวลาไซต์',
  'kpi.orderEnd.source.note':
    'ตัวเลขนี้จะเพิ่มขึ้นเรื่อย ๆ ระหว่างวันและเริ่มนับใหม่ตอนเที่ยงคืน จึงมากกว่าจำนวนเครื่องข้าง ๆ ได้\n' +
    'เครื่องจักรทั้งหมดตั้งใจไม่นับใบเหล่านี้ เพราะเครื่องที่ผลิตจบใบหนึ่งแล้วเดินใบถัดไปอยู่จะถูกนับซ้ำสองครั้ง',

  'kpi.machines.source': 'นับจากสถานะล่าสุดที่แต่ละเครื่องส่งเข้า InfluxDB',
  'kpi.machines.source.formula': 'TOTAL = RUNNING + STOP',
  /* แก้ 2026-09-10 - ดู en.ts · ข้อความเดิมบอกว่า No Plan กับ Pending ไม่ถูกนับ
     ซึ่งขัดกับการ์ด Stop ที่อยู่ห่างไปสองใบ */
  'kpi.machines.source.note':
    'มีแต่ Order End ที่ไม่ถูกนับ เพราะบอร์ดหน้างานแสดงใบสั่งที่ผลิตจบเป็นการ์ดอีกใบคู่กับใบที่เครื่องกำลังเดินอยู่ ถ้านับด้วยจะกลายเป็นนับเครื่องเดียวกันสองครั้ง · สถานะอื่นที่ไม่ได้เดินถูกนับอยู่ในการ์ด Stop ทั้งหมด รวม No Plan และ 4M Change · ส่วนฐานที่ไม่ส่งข้อมูลจะไม่ถูกนับเลย',
  'kpi.running.source':
  ...
  /* ------------------------------------------------------------ alerts */
  'alerts.title': 'เครื่องที่หยุดอยู่นานที่สุด',
  'alerts.sub': 'หยุดอยู่ {n} เครื่อง',
  ...
  'common.grafanaPending': 'ยังไม่มีแดชบอร์ด Grafana ของไซต์นี้',
  'country.TH': 'ไทย',
  'country.JP': 'ญี่ปุ่น',
  'country.HU': 'ฮังการี',
  'country.VN': 'เวียดนาม',
