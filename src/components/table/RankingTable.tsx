...
import type { CompanySummary, GlobalOverview, PlantSummary, QtyUnit } from '../../api/contract';
 ...
 *
 * Takes `t` as an argument rather than reaching for `useI18n` itself: it is
 * called from inside a `.map()` in the table body, where a hook cannot go.
 */
function countryName(code: string, t: TFunction): string {
...
export function RankingTable({ data }: { data: GlobalOverview | undefined }) {
  ...
  /* Handed to every column head, so the heads stay presentational and the
     transition - which column, and which way round - lives with the state. */
  const onSort = (col: SortCol) => setSort((s) => nextSort(s, col));
  ...
  return (
    <div className="panel__scroll">
      <table className="rank-table rank-table--fill">
        ...
        <thead>
          <tr>
            <SortHead col="code" labelKey="table.base" className="col-id" sort={sort} onSort={onSort} />
            ...
            <SortHead col="oa" labelKey="table.oa" className="num" sort={sort} onSort={onSort} />
            <SortHead col="achv" labelKey="table.achv" className="num" sort={sort} onSort={onSort} />
            <SortHead col="down" labelKey="table.down" className="num" sort={sort} onSort={onSort} />
                      ...
                      <tr key={c.code} className="row-quiet">
                        <td className="col-id">
                          <span className="rank-id">
                            ...
                            <Flag code={c.country_code} countryName={countryName(c.country_code, t)} />
...
/* ========================================================================
 * The table's parts - at module scope, and they have to stay there.
 *
 * Every one of these was declared INSIDE RankingTable, closing over `t`,
 * `sort`, `link` and `data` instead of taking them as props. That reads as the
 * tidier arrangement and it is a remount bug: a function declared in a render
 * body is a NEW function on every render, React compares element types by
 * identity, and a different type at the same position is not an update - it is
 * an unmount followed by a mount. Every row in this table was therefore thrown
 * away and rebuilt from scratch on each render.
 *
 * This table re-renders once a second by design (`nowMs`, so the nine clocks
 * tick) and again on every 5s poll, so that was up to nine rows of DOM
 * destroyed and recreated every second. What made it visible is the stale
 * clock: `MeasureValue` opens the "as of 8 minutes ago" age in a native
 * popover, and a popover whose element is removed from the document closes.
 * The reader tapped the clock, the next tick deleted the element under it, and
 * the age vanished on its own about a second later - reported as "the yellow
 * clock disappears by itself". Anything else the DOM owns rather than React -
 * focus inside a row, a text selection, an :active button - was being dropped
 * the same way, once a second, silently.
 *
 * The props below are what the closures used to supply. Threading `sort`,
 * `nowMs` and `qtyUnit` down is the price of stable component identity, and it
 * is the whole fix: nothing here memoises, and nothing needs to.
 * ======================================================================== */
...
function PlantRow({
  ...
  return (
    <tr id={id} className="row-plant">
       ...
       */}
      <td className="num">
        <Output actual={plant.kpi.actual_qty} plan={plant.kpi.plan_qty} qtyUnit={qtyUnit} />
      </td>
 ...
 *
 * The unit is a prop rather than a second read of the payload: it is a property
 * of the window every row was measured in (section 8.5 - Shot and Pcs get
 * swapped in the source), so it is passed down from the one place that holds
 * the payload instead of each row reaching back for it.
 */
function Output({
  actual,
  plan,
  ...
  return (
    <span className="output">
      ...
      <span className="quiet">{t(`unit.${qtyUnit}` as TKey)}</span>
    </span>
  );
}
