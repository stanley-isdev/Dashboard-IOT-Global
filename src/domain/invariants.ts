import type { Counts, GlobalOverview, MachineStatus } from '../api/contract';
import { isReporting } from './status';

/**
 * Runtime checks on the shape of the truth, not the shape of the JSON.
 *
 ...
 * somewhere that sends no data.
 *
 * The design doc's section 16 asks for a one-off reconciliation against Grafana
 * before go-live. This turns that into a check that runs on every poll, which
 * matters because the `Order End` status is derived in two separate layers and
 * a partial reimplementation will not look wrong - it will just be wrong.
 *
 * In development these throw. In production they accumulate and surface as
 * "N data integrity warnings" in the footer, because a dashboard that blanks
 * itself over an arithmetic disagreement is less useful than one that shows the
 * numbers and says it is unsure.
 */
...
function checkPartition(counts: Counts, where: string, out: Violation[]): void {
...
export function checkGlobalOverview(payload: GlobalOverview): Violation[] {
  ...
  // Provenance: the mockup lists a heater failure at VNS and a jam at ISE, both
  // of which are declared as having no telemetry. A site that sends nothing
  // cannot report a fault.
...
/**
 * Called from the query layer. Throws in development so a bad payload cannot be
 * ignored; returns the list in production so the UI can flag it without going
 * blank.
 */
export function assertOrCollect(violations: Violation[], context: string): Violation[] {
  if (violations.length === 0) return violations;

  const summary = violations.map((v) => `  [${v.rule}] ${v.where}: ${v.detail}`).join('\n');
  if (import.meta.env.DEV) {
    console.error(`Data integrity violations in ${context}:\n${summary}`);
  }
  return violations;
}
