...
## 0. อัปเดต 2026-09-11 - เก็บ SQL + JS ของ v4 ไว้ในรีโปแล้ว

> `docs/grafana/panel-v4-asi.sql` (คิวรี่เต็ม) และ `docs/grafana/panel-v4-asi.js`
> (เฉพาะท่อนที่ตัดสินตัวเลข) - ถอดจาก panel สดของบอร์ด ASI `adz5fll` วันที่ 2026-09-11

§2.1 ข้างล่างคือ **v3** (จับภาพ 2026-08-27) ยังเก็บไว้เพื่อเทียบ แต่เวลาจะ reconcile
ให้ยึดสองไฟล์ข้างบนเป็นหลัก ระหว่างสองวันนั้นมีอย่างน้อยสามกฎที่เปลี่ยนและอยู่ในตัวสคริปต์เท่านั้น
ไม่มีที่ไหนบันทึกไว้เลย - รอบนี้เลยเก็บเป็นไฟล์ ไม่ใช่สรุปในเอกสาร

**สิ่งที่คลี่ออกได้ทันทีจากไฟล์พวกนี้:**

1. **การ์ด `Pending`/`Order End` ไม่ถูก render เป็นการ์ดเลย** - content ออกมาเป็น
   `.nav-marker` ที่ซ่อนไว้ แล้วนับขึ้น chip ที่ลิงก์ไปแดชบอร์ดแยก (`order-end-v1-0`,
   `pending-v1-0`) นี่คือคำตอบของ F-19 ว่าทำไม `M-ID-06` ถึงไม่มีบนบอร์ด
2. **`Dandori` แยกออกจาก RUNNING แล้วใน v4** - ดู F-20 ฝั่งเรายังรวมอยู่
3. **หน้าต่างสถานะของบอร์ด ASI คือ 3 วัน ไม่ใช่ 1 วัน** - ทั้ง `RealtimeStatus_Latest`
   และ `StatusPerPO_Latest` ส่วนของเราใช้ช่วง range ที่ผู้ใช้เลือก (ปกติ 24 ชม.)
   เครื่องที่เงียบไป 1-3 วันจะอยู่บนบอร์ดแต่ไม่อยู่ในสำมะโนของเรา ตอนนี้ที่ 6051 ไม่มีเครื่องไหนเข้าเคสนี้
4. **F-17 ยืนยันแล้ว** - machine exclusion hardcode อยู่ใน CTE ฝั่งสถานะสองตัว
   ไม่อยู่ในฝั่ง IO จริงตามที่เคยบันทึกไว้

**ข้อควรรู้ก่อนเทียบตัวเลขครั้งต่อไป - เราอ่านคนละ instance กับบอร์ด**

ยืนยันโดยเจ้าของงาน 2026-09-11: InfluxDB ที่ backend นี้อ่าน (`INFLUX_DATABASE`
ใน `server/.env`) **ไม่ใช่ตัวเดียวกับที่บอร์ดหน้างานอ่าน** - สอดคล้องกับ §4.8b ที่
เคยบันทึกไว้ตั้งแต่เคส STJ

ผลต่อการ reconcile: ตัวเลขที่ไม่ตรงกันจะแยกไม่ออกระหว่าง "กฎเราผิด" กับ "ข้อมูล
สองฝั่งไม่เหมือนกัน" จนกว่าจะเช็คชุดเครื่องก่อน ตัวอย่างที่เจอวันนี้: `IA1` `IA2`
`P1I4` มีอยู่ที่ 6332 ใน instance ของเรา และ SQL ของบอร์ด ASI ก็มีบรรทัดตัดสาม
ชื่อนี้ออกค้างอยู่ - ดูเหมือนต่างกัน 3 เครื่อง แต่เจ้าของงานชี้ว่ามาจาก instance
คนละตัว ไม่ใช่กฎที่เราพลาด **จึงไม่ได้ใส่ `machineExclusions` ที่ 6332** และ
ไม่ควรใส่จนกว่าจะเทียบบนข้อมูลชุดเดียวกันได้

กฎที่เทียบแล้วตรงจริง (%OA, TOTAL, หน้าต่างสำมะโน) ตรงกันบนไซต์ที่ข้อมูลสองฝั่ง
ตรงกันอยู่แล้ว - 6051 เป็นตัวที่ใช้ยืนยันทุกข้อในวันนี้
...
`machineOaQueries` รับ `SiteWindowPlan` แล้ว emit เป็น **SELECT หนึ่ง query ต่อหนึ่งหน้าต่าง** โดยแต่ละ
สาขาถูกจำกัดด้วย `plant IN (...)` ของตัวเอง — ไม่ซ้อนกัน (แต่ละ plant อยู่สาขาเดียว)

> ⚠️ **เคยเป็น `UNION ALL` รวมทุกสาขาไว้ใน query เดียว และนั่นคือสิ่งที่ทำให้บอร์ดดับทั้งใบ
> เมื่อ 2026-09-16** — เพดาน scan ไฟล์ของ InfluxDB นับ "ต่อ 1 query" ดังนั้น union ของสาขา 71 ชม.
> (ASI) กับสาขา 24 ชม. (ที่เหลือ) จะถูกคิดค่าตามสาขา 71 ชม. พอโดนปฏิเสธ
> (`Query would scan 432 Parquet files, exceeding the file limit`) ก็ล้มทั้ง statement
> ทั้ง 9 โรงงานขึ้น `No data` พร้อม banner ว่าต่อ InfluxDB ไม่ได้ — ทั้งที่ instance ตอบ query 24 ชม.
> ได้ใน 138 ms
>
> **ตัวกรอง plant ไม่ช่วยลดจำนวนไฟล์เลย** — วัดวันเดียวกันที่ความกว้างที่กำลังโดนปฏิเสธ (84 ชม.)
> ทั้ง `plant IN ('6051')`, `plant NOT IN ('6051')` และแบบไม่ใส่ตัวกรองเลย ถูกปฏิเสธเหมือนกันหมด
> ขณะที่ช่วงเดียวกันซอยเป็น 4 ช่วง ๆ ละ 24 ชม. ตอบได้ครบทุกช่วง (3,629 / 3,884 / 2,676 / 2,928 แถว)
> เพดานนับ "ไฟล์" ไฟล์เรียงตาม "เวลา" ฉะนั้นมีแต่ **จำนวนชั่วโมง** เท่านั้นที่ขยับมันได้
>
> การแยก query จึงจำกัดความเสียหายให้อยู่แค่ไซต์เดียว และ `SiteQuery.narrow()` (ซอยเป็นช่วง
> `NARROW_WINDOW_HOURS`) คือสิ่งที่ดึงข้อมูลของไซต์นั้นกลับมา — ดู `server/test/siteSplit.test.ts`

poller ยิงสาขาเหล่านี้ **ทีละอัน ไม่ขนาน** ซึ่งเป็นกฎเรื่องความถูกต้อง ไม่ใช่ความสุภาพ: instance นี้
ตอบ concurrent read ด้วย HTTP 200 แต่ **แถวหายไปเฉย ๆ** (วัด 2026-09-08: ยิงพร้อมกัน 6 อัน ได้ 828 แถว
เทียบกับ 1,510 แถวเมื่อยิงทีละอัน) ซึ่งฝั่งเราแยกไม่ออกจาก "วันนี้งานน้อย"
...
| **F-18** ✅ ปิดแล้ว | **`Pending` (v4) ไม่ถูกกันออกจาก `%OA` เฉลี่ยฝั่ง backend** | Grafana v4 ตัด `Pending` ออกจากทั้ง Total และ Avg %OA (`EXCLUDE_FROM_TOTAL`/`EXCLUDE_FROM_OA` ใน §0) แต่ `server/src/domain/oa.ts` ไม่รู้จักสถานะเครื่องเลย - คำนวณจาก `production_machine_io` ล้วน ไม่ join กับ `production_machine_status.Result` จึงนับเครื่องที่ถูกกด `Pending` ค้างไว้เข้าค่าเฉลี่ยด้วย ทั้งที่ Grafana ไม่นับ ผู้ต้องสงสัยหลักของช่องว่าง %OA plant 6051 (51.2% vs 50.9%, 2026-09-10) | §0, `server/src/domain/oa.ts`, `server/src/influx/queries.ts` `SUBSTANTIVE_STATUSES` |
| **F-19** ✅ ปิดแล้ว | **`Order End` เป็นค่าที่ถูกเขียนลง `Result` จริง และบอร์ดย้ายการ์ดนั้นออกไปหน้าอื่น** | วัดที่ ASI 6051 เมื่อ 2026-09-11: `M-ID-06` มีแถว `Result = 'Order End'` ใน `production_machine_status` (08:46 น.) หลังใบสั่ง `110000994598` หมดและยังไม่มีใบใหม่ขึ้น · `M-ID-01` ขึ้น `Order End` แวบเดียว 2 วินาทีตอนสลับใบสั่ง → เป็นสถานะที่ Node-RED เขียนเอง ไม่ใช่ derived อย่างเดียว · **คำตอบว่าทำไมบอร์ดไม่มีการ์ดนั้น อยู่ใน JS v4 ที่เพิ่งเก็บมา (2026-09-11)**: content template **ไม่ render การ์ดให้สถานะ `Pending`/`Order End` เลย** มีแค่ `<span class="nav-marker">` ที่ซ่อนไว้ แล้ว summary bar นับ marker นั้นขึ้น chip ที่ลิงก์ไปแดชบอร์ดแยกของแต่ละสถานะ (`order-end-v1-0`, `pending-v1-0`) → เครื่องไม่ได้หาย แต่ถูกย้ายออกจากหน้าหลัก · ผลพลอยได้: `EXCLUDE_FROM_TOTAL` เป็นแค่กันเหนียว เพราะสถานะที่ไม่มีการ์ดนับเข้า `counts` ไม่ได้อยู่แล้ว · **ฝั่งเราปิดครบแล้ว**: `oaExcludedMachines` ตัด `Order End`/`Pending`/`Offline`/`No Plan` ออกจาก %OA (6051: 45.8% → 34.8%) และ `EXCLUDED_FROM_TOTAL` รับ `Pending` เข้ามา · **ต่างที่เหลือโดยตั้งใจ**: รายการเครื่องของเรายังแสดงการ์ดนั้นอยู่ เพราะเราไม่มีแดชบอร์ดแยกให้คลิกไป ซ่อนแล้วข้อมูลจะหายไปเฉย ๆ | `panel-v4-asi.sql`, `panel-v4-asi.js`, `server/src/domain/oa.ts`, `server/src/domain/counts.ts` |
| **F-20** ✅ ปิดแล้ว | **บอร์ด v4 แยก `Dandori` ออกจาก RUNNING แล้ว ฝั่งเรายังรวมอยู่** | v3 คิด `running = counts['Mass Pro'] + counts['Dandori']` · **v4 เปลี่ยนเป็น `running = counts['Mass Pro']` ล้วน แล้วให้ `Dandori` เป็น tile ของตัวเอง** (ดู `panel-v4-asi.js` step 9) ส่วน `BUCKET_OF` ยังจัด `Dandori` เข้า `running` ตามกฎ v3 · วัดที่ 6051 วันที่ 2026-09-11: Mass Pro 34 · Dandori 3 → บอร์ดขึ้น Running 34 / Dandori 3 ส่วนเราขึ้น 37 · **ตัดสินใจ 2026-09-11 (design owner): คงนิยาม `running` = Mass Pro + Dandori ไว้** เพราะเครื่องที่กำลังเปลี่ยนแม่พิมพ์คือเครื่องที่มีคนทำงานอยู่ ไม่ใช่เครื่องหยุด **แต่ให้แจกแจงบนหน้าการ์ด** ทั้งสามจอ (`KpiStrip`, `CompanyPage`, `PlantPage` ซึ่งทำอยู่แล้ว) เป็น `Mass Pro 34 · Dandori 3` เพื่อให้คนที่วางสองจอข้างกันอ่านออกทันทีว่า 37 ของเราคือเลขเดียวกับ 34/3 ของบอร์ด ไม่ได้แก้ `BUCKET_OF` และไม่ได้แตะ contract | `panel-v4-asi.js`, `src/components/kpi/KpiStrip.tsx`, `src/pages/CompanyPage.tsx` |
| **F-21** ✅ ปิดแล้ว | **หน้าต่างสำมะโนของบอร์ด ASI คือ 3 วัน ของเราเป็น 24 ชม. ค่าเดียวทั้งฟลีต** | `RealtimeStatus_Latest` กับ `StatusPerPO_Latest` ใน `panel-v4-asi.sql` bound ที่ `now() - INTERVAL '3 days'` ส่วนบอร์ด THS (§2.1) ใช้ `1 days` · เครื่องที่เงียบนานกว่าหน้าต่างของบอร์ดตัวเองจะไม่มีการ์ดและไม่อยู่ในยอดนับใด ๆ ของบอร์ดนั้น ฝั่งเราใช้ `HOT_WINDOW_HOURS = 24` ค่าเดียวทั้งฟลีต สำมะโนของ ASI จึงแคบกว่าบอร์ดของ ASI เอง · **วัดที่ 6051 เมื่อ 2026-09-11**: `M-IS-38` ส่งข้อมูลล่าสุดเป็น `Dandori` เมื่อ 2 วันก่อน บอร์ดนับ 43 เครื่อง (Dandori 4) เราได้ 42 (Dandori 3) ไม่ได้เกี่ยวกับกฎ `Order End` เลย เป็นเรื่องหน้าต่างล้วน ๆ · **แก้แล้ว**: เพิ่ม `statusWindowHours` เป็นค่าราย company ใน master data (ASI = 71, ที่เหลือ = 24) แบบเดียวกับ `oaWindowHours` และแยก `perSiteWindowSql` ออกมาให้ทั้งคิวรี่ %OA และคิวรี่สำมะโนใช้แผนเดียวกัน - **ไม่ทำเป็นค่าเดียวทั้งระบบ** เพราะ 71 ตัวเดียวเคยลาก THS ออกจากบอร์ดของ THS มาแล้วเมื่อ 2026-09-10 · ผลหลังแก้: 6051 TOTAL 42 → **43** ตรงกับบอร์ด, THS 6332 ไม่ขยับ (30) | `panel-v4-asi.sql`, `server/src/config/masterData.ts`, `server/src/influx/queries.ts`, `server/src/app.ts` |
