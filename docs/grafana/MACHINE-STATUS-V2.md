...
| **F-19** | **`Order End` เป็นค่าที่ถูกเขียนลง `Result` จริง และการ์ดนั้นหายไปจากบอร์ด** | วัดที่ ASI 6051 เมื่อ 2026-09-11: `M-ID-06` มีแถว `Result = 'Order End'` ใน `production_machine_status` (08:46 น.) หลังใบสั่ง `110000994598` หมดและยังไม่มีใบใหม่ขึ้น · `M-ID-01` ก็ขึ้น `Order End` แวบเดียว 2 วินาทีตอนสลับใบสั่ง → เป็นสถานะที่ Node-RED เขียนเอง ไม่ใช่ derived อย่างเดียวแบบชั้นที่ 1 · **ผู้ใช้ยืนยันด้วยตาว่าบอร์ดไม่มีการ์ด `M-ID-06` เลย** ทั้งที่ SQL §2.1 ที่บันทึกไว้ (จับภาพ 2026-08-27) รันกับ DB ตัวเดียวกันแล้ว**ยังคืนแถวนั้นออกมาเป็นการ์ด `Order End`, seq = 1** และ `FULL JOIN` แปลว่าเครื่องที่มีแต่ฝั่งสถานะก็ต้องได้การ์ด → กลไกที่ซ่อนการ์ดอยู่ใน **v4 ที่ยังไม่ได้เก็บ SQL/JS ฉบับเต็ม** (§0 บันทึกไว้แค่ท่อน `CASE` กับ `EXCLUDE_FROM_*`) · **ฝั่งเราปิดครึ่งหนึ่งแล้ว**: `oaExcludedMachines` ตัดทั้ง `Order End`/`Pending`/`Offline`/`No Plan` ออกจาก %OA ตาม `EXCLUDE_FROM_OA` - 6051 ขยับจาก 45.8% (7 เครื่อง) เป็น 34.8% (6 เครื่อง) · **ยังค้าง**: รายการเครื่องของเรายังแสดงการ์ดนั้นอยู่ รอ SQL v4 ฉบับเต็มมาตัดสินว่าบอร์ดซ่อนด้วยกฎอะไร | §0, §2.1, `server/src/domain/oa.ts`, `server/src/services/globalOverviewService.ts` |
