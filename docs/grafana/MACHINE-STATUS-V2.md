...
| **F-20** | **บอร์ด v4 แยก `Dandori` ออกจาก RUNNING แล้ว ฝั่งเรายังรวมอยู่** | v3 คิด `running = counts['Mass Pro'] + counts['Dandori']` · **v4 เปลี่ยนเป็น `running = counts['Mass Pro']` เพียว ๆ แล้วให้ `Dandori` เป็น tile ของตัวเอง** (ดู `panel-v4-asi.js` §step 9) ส่วน `BUCKET_OF` ใน `packages/domain-shared/src/statusBucket.ts` ยังจัด `Dandori` เข้า `running` ตามกฎ v3 · วัดที่ 6051 วันที่ 2026-09-11: Mass Pro 32 · Dandori 2 → บอร์ดขึ้น Running 32 / Dandori 2 ส่วนเราขึ้น running 34 · ตัวเลขพาดหัวที่คนเอาไปเทียบกันตรง ๆ **ยังไม่ได้แก้ รอตัดสินใจ** ว่าจะแยก tile ตามบอร์ด หรือคงนิยาม running เดิมไว้ | `panel-v4-asi.js`, `packages/domain-shared/src/statusBucket.ts` |
