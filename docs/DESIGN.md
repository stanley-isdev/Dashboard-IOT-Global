...
| Map engine | Leaflet.js 1.9.4 + CartoDB Positron (`light_all`) |
...
| # | ปัญหา | ความสำคัญ |
|---|---|---|
| T-01 | KPI "Global Achievement" ยัง hardcode `Plan 14,200 / Actual 11,590` | สูง |
...
```sql
TotalPlan    = plan_qty0 + plan_qty1 + plan_qty2 + plan_qty3
OutputActual = SUM(qty)                       -- ต่อ Group_PO × machine
AR_percent   = (OutputActual / TotalPlan) * 100   -- ถ้า TotalPlan = 0 → 0
```
...
| Q-03 | %OA เฉลี่ยต่อ plant → company → global | ต้องปิด D-20 ก่อน |
| Q-04 | %Achievement roll-up (Σplan / Σactual) | ระวังนับ plan ซ้ำเมื่อ PO เดียวอยู่หลายเครื่อง |
| Q-05 | Trend %OA 24 ชม. แบ่ง bucket 1 ชม. | `date_bin(INTERVAL '1 hour', time)` |
...
| D-02 | **สูตร %OA / %AR** | ✅ **ปิดแล้ว** — ข้อ 9 (เหลือ D-19, D-20) |
...
| D-20 | **วิธี aggregate Global %OA** — simple average vs weighted | ⬜ **ใหม่** |
| D-21 | `4M Change` นับเป็น Running หรือ Stopped ใน KPI | ⬜ **ใหม่** |
| D-22 | Threshold `std_time+20` vs `std_time+100` ตั้งใจหรือ bug | ⬜ **ใหม่** |
...
| D-26 | Bucket รายชั่วโมง — shift-relative (ชม.ที่ 1,2,3 ของกะ) หรือ clock-time ระดับนาที | ⬜ **ใหม่** · แนะนำ shift-relative |
