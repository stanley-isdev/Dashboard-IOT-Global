...
| %OA Target | ค่าเริ่มต้น 100% ควบคุมผ่าน `TARGET_OA` จุดเดียว |
...
> ### 🔴 กฎใหม่ของ layer 2 (เจ้าของ design doc, 2026-09-08) - **จัดประเภทได้ แต่ห้ามตัดข้อมูลออก**
>
> layer 2 **ห้ามตัดเครื่องออกจากค่าเฉลี่ย %OA** อีกต่อไป งานจะถือว่าจบก็ต่อเมื่อ **ช่อง `ProductionOrderN` กลับไปเป็น `-`** เท่านั้น ซึ่งเป็นสิ่งที่ gateway บอกมาตรงๆ ไม่ใช่สิ่งที่เราอนุมานเอง ห้ามเดาว่า "order สร้างก่อนกะนี้ = จบแล้ว" แล้วเอาตัวเลขของเครื่องนั้นออก
>
> **เหตุผล - วัดที่ ASI 6051 วันที่ 2026-09-08 เวลา 17:45 น.** ASI เดิน order เดียวข้ามวัน เครื่องที่คำนวณ %OA ได้ 7 ตัว (เฉลี่ยจริง **75.0%** และทุกตัวยังยิงอยู่ ณ นาทีนั้น) ถูกตัดสินเป็น `ended` **ครบทั้ง 7 ตัว** เพราะ `vCreateDateTxt` = `2026/09/07 09:5x` คือเช้าวันก่อน → การ์ด %OA ของทั้งโรงว่างเปล่า ส่วน THS ออก order ใหม่ทุกกะเลยไม่เคยเจอปัญหานี้ กฎที่ reconcile มาจาก THS จึงถูกเข้าใจผิดว่าใช้ได้ทุกไซต์
>
> **ผลที่ยอมรับแล้ว:** ตัวเลข %OA ของบอร์ดนี้จะ**ต่ำกว่า**บอร์ด `Machine Status V2.0` เดิม เท่ากับจำนวนเครื่องที่ถือ order ข้ามกะพอดี (THS: 75.2% ที่นี่ เทียบกับ 81.0% ที่บอร์ดเดิม) โดยระบุชื่อเครื่องเหล่านั้นไว้ใน `meta.warnings` ราย plant ทุกครั้ง
>
> ชั้นที่ 1 (`global_machine_seq > 1`) ไม่กระทบ - ยังตัดออกจาก TOTAL ตามเดิม เพราะเป็นการ์ดใบเพิ่มที่บอร์ดสร้างขึ้นต่อ order ที่จบแล้ว ไม่ใช่การ์ดของตัวเครื่องเอง
